# App Split — Feature Modules + st.navigation() Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Split the 1161-line `app.py` monolith into focused page modules under `pages/`, replacing the manual `session_state.page` router with Streamlit's native `st.navigation()`.

**Architecture:** Each feature lives in `pages/<feature>.py` as a `render()` function called at module level. `app.py` becomes a ~50-line orchestrator: config, CSS, session init, sidebar, auth gate, and `st.navigation()` with file-based `st.Page("pages/X.py")` references. File-based pages enable `st.switch_page("pages/X.py")` from any module without circular imports.

**Tech Stack:** Python 3.x, Streamlit ≥ 1.35, existing `tools/shared/` utilities unchanged.
...
| Create | `pages/_shared.py` | `PROJECT_ROOT`, `_load_kb()`, `PDPA_NOTICE`, `_gsheets()`, `_identity()`, `_claude()` |
| Create | `pages/login.py` | Login / register form |
| Create | `pages/dashboard.py` | Activity overview, topic mastery, getting-started |
| Create | `pages/chatbot.py` | Q&A chat, session save, flash-card generation trigger |
| Create | `pages/flashcards.py` | SM-2 spaced-repetition review loop |
| Create | `pages/cases.py` | Case selection, active case, results + XP |
| Create | `pages/image_quiz.py` | Retinal image description + evaluation |
| Create | `pages/admin.py` | Cost monitor, backup, schema check, health check |
| Rewrite | `app.py` | Config, CSS, session init, sidebar, auth gate, router |

---

## Task 1: Create pages package + _shared.py

**Files:**
- Create: `pages/__init__.py`
- Create: `pages/_shared.py`
...
- [ ] **Step 2: Create `pages/_shared.py`**
...
```
python -c "from pages._shared import _load_kb, PDPA_NOTICE, _gsheets, _identity, _claude; print('OK')"
```
Expected: `OK`
...
```
git add pages/__init__.py pages/_shared.py
git commit -m "feat: add pages package with shared utilities"
```

---

## Task 2: Create pages/login.py

**Files:**
- Create: `pages/login.py`

- [ ] **Step 1: Create `pages/login.py`**
...
Note: `st.rerun()` after login triggers `app.py` to re-evaluate the auth gate; the full navigation with Dashboard as default loads automatically — no `session_state.page` assignment needed.

- [ ] **Step 2: Commit**

```
git add pages/login.py
git commit -m "feat: extract login page module"
```

---

## Task 3: Create pages/dashboard.py

**Files:**
- Create: `pages/dashboard.py`

- [ ] **Step 1: Create `pages/dashboard.py`**
...
git commit -m "feat: extract dashboard page module"
```

---

## Task 4: Create pages/chatbot.py

**Files:**
- Create: `pages/chatbot.py`

- [ ] **Step 1: Create `pages/chatbot.py`**
...
```
git add pages/chatbot.py
git commit -m "feat: extract chatbot page module"
```

---

## Task 5: Create pages/flashcards.py

**Files:**
- Create: `pages/flashcards.py`

- [ ] **Step 1: Create `pages/flashcards.py`**
...
```
git add pages/flashcards.py
git commit -m "feat: extract flashcards page module"
```

---

## Task 6: Create pages/cases.py

**Files:**
- Create: `pages/cases.py`

This is the largest page (~420 lines in the original). The nested `_level_banner()` function is promoted to module-level private.

- [ ] **Step 1: Create `pages/cases.py`**
...
```
git add pages/cases.py
git commit -m "feat: extract cases page module"
```

---

## Task 7: Create pages/image_quiz.py

**Files:**
- Create: `pages/image_quiz.py`

- [ ] **Step 1: Create `pages/image_quiz.py`**
...
```
git add pages/image_quiz.py
git commit -m "feat: extract image quiz page module"
```

---

## Task 8: Create pages/admin.py
...
git commit -m "feat: extract admin page module"
```

---

## Task 9: Rewrite app.py

**Files:**
- Modify: `app.py` (full rewrite — back up the original first)
...
```
git add app.py
git commit -m "refactor: replace monolith app.py with st.navigation() orchestrator"
```
...
- [ ] **Step 1: Verify shared module imports cleanly**

```
python -c "from pages._shared import _load_kb, PDPA_NOTICE, _gsheets, _identity, _claude, PROJECT_ROOT; print('shared OK')"
```
Expected: `shared OK`
...
Navigate to each page in the browser and confirm it renders without errors:
- [ ] Login page loads, form visible
- [ ] Login with a test name/email succeeds, redirects to Dashboard
- [ ] Dashboard loads, stat cards visible
- [ ] Chatbot Tutor loads, chat input visible
- [ ] Flash-cards loads, either cards or "all caught up" message visible
- [ ] Case Simulator loads, case cards visible
- [ ] Image Quiz loads, image selector visible
- [ ] Admin loads, expanders visible
- [ ] Logout button resets to Login page
...
del app.py.bak
git add -A
git commit -m "chore: remove app.py backup after verified smoke test"
```
